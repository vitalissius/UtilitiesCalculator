...
public class StatisticsReadWriter {
    ...
    public synchronized void write(Statistics statistics) {
        StandardOpenOption[] options = { StandardOpenOption.CREATE, StandardOpenOption.APPEND };

        try (BufferedWriter writer = Files.newBufferedWriter(Settings.UC_STATISTICS_FILE_PATH, options)) {
    ...
    public synchronized List<Statistics> read() {
        List<Statistics> stats = new ArrayList<>();

        Pattern pattern = Pattern.compile(RGX_FILE_LINE_FORMAT);

        try (BufferedReader reader = Files.newBufferedReader(Settings.UC_STATISTICS_FILE_PATH)) {
